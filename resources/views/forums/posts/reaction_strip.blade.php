@if (auth()->check())
    ...
    <button id="love" data-post-id="{{ $post->id }}" data-user-id="{{ auth()->user()->id }}" data-tooltip="Love" class="post-likes reaction">
        ...
        @if ( $post->reactions->where('reaction', 'love')->count() == 0)
            <span class="total hide">{{ $post->reactions->where('reaction', 'love')->count() }}</span>
        @else
            <span class="total">{{ $post->reactions->where('reaction', 'love')->count() }}</span>
        @endif
    </button>
    ...
    <button id="angry" data-post-id="{{ $post->id }}" data-user-id="{{ auth()->user()->id }}" data-tooltip="Angry" class="post-likes reaction">
